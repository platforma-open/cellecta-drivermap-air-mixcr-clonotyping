...
import polars as pl
import polars_hash  # noqa: F401 -- side-effect import: registers the `.chash` expression namespace used below
...
def parse_calculate_args(calculate_args: List[List[str]]) -> List[Tuple[List[str], str]]:
    ...
    for i, calc_arg in enumerate(calculate_args):
        if len(calc_arg) < 2:
            print(
                f"Error: --calculate argument #{i + 1} must have at least one input "
                f"column and one output column name. Got: {calc_arg}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def main():
    ...
    try:
        ...
        for input_cols, output_name in calculation_specs:
            ...
            if output_name in df.columns:
                print(
                    ...
                    f"already exists in the input table. It will be overwritten.",
                    file=sys.stderr,
                )
        ...
        if missing_input_cols:
            print(
                f"Error: The following input columns required by --calculate arguments "
