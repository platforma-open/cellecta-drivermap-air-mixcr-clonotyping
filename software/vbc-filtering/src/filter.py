...
def main(input_file, output_prefix):
    ...
    for barcode_count in range(1, 9):
        ...
        if not subset.empty:
            thresholds[barcode_count], left_maxes[barcode_count], right_maxes[barcode_count] = (
                find_kde_mimima_threshold_2(subset, barcode_count, output_prefix)
            )
    ...
    with open(f"{output_prefix}.kde.maximas.txt", "a") as f:
        for barcode_count in range(1, 9):
            f.write(
                f"{barcode_count}\t{thresholds[barcode_count]}\t{left_maxes[barcode_count]}\t{right_maxes[barcode_count]}\n"
            )

    # Apply thresholds to clones
    clone_total_reads = clone_total_reads.copy()
    clone_total_reads.loc[:, "keep"] = clone_total_reads.apply(
        lambda row: row["readCount"] >= thresholds.get(row["barcode_count"], np.inf), axis=1
    )
