...
import {
  AlignmentChainColors,
  ImmuneChains,
} from "@platforma-open/milaboratories.mixcr-clonotyping-2.model";
import type { PlChartStackedBarSettings } from "@platforma-sdk/ui-vue";
        ...
        : ImmuneChains.flatMap((chain) => {
            ...
            return s
              ? Object.entries(pick(s, "total", "hasStops", "isOOF")).map(([key, value]) => {
                  return {
                    color:
                      AlignmentChainColors[chain]?.[key as "total" | "hasStops" | "isOOF"] ??
                      "#000",
